...
export default function Backups() {
  ...
  const { openConfirm: openConfirmDel, ConfirmChild: ConfirmDelChild } = useConfirm({
    content: t('confirm_delete_the_backup') as I18nCommonKey
  });
  ...
  const columns = useMemo<Array<ColumnDef<BackupItemType>>>(
    () => [
      ...
      },
      {
        ...
        accessorKey: 'type',
        header: () => t('Type'),
        cell: ({ row }) => t(backupTypeMap[row.original.type]?.label as I18nCommonKey) || '-'
      },
      {
        id: 'actions',
        header: () => t('operation'),
    ...
    [confirmDel, openConfirmDel, t, loadDBDetail]
  );
